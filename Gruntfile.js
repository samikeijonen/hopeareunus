...
grunt.initConfig({
  pkg: grunt.file.readJSON('package.json'),
	...
	uglify: {
		shiv: {
			files: {
				'js/html5shiv/html5shiv.min.js': ['js/html5shiv/html5shiv.js']
			}
		},
		fitvids: {
			files: {
		...
		settigns: {
			files: {
				'js/settings/settings.min.js': ['js/settings/settings.js']
			}
		},
		navigation: {
			files: {
				'js/navigation/navigation.min.js': ['js/navigation/navigation.js']
			}
		}
	},
	...
	replace: {
		...
		functionsVersion: {
			...
			overwrite: true,
			replacements: [ {
				from: /^define\( 'HOPEAREUNUS_VERSION'.*$/m,
				to: 'define( \'HOPEAREUNUS_VERSION\', \'<%= pkg.version %>\' );'
			} ]
		}
	},
